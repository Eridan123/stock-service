...
public class ElasticSearchQuery {
    ...
    private final String indexName = "stocks-index";


    public String createOrUpdateDocument(ElasticStock product) throws IOException {
    ...
    public ElasticStock getDocumentById(String productId) throws IOException{
        ElasticStock product = null;
        GetResponse<ElasticStock> response = elasticsearchClient.get(g -> g
                        .index(indexName)
                        .id(productId),
                ElasticStock.class
        );
    ...
    public  List<ElasticStock> searchAllDocuments() throws IOException {

        SearchRequest searchRequest =  SearchRequest.of(s -> s.index(indexName));
        SearchResponse searchResponse =  elasticsearchClient.search(searchRequest, ElasticStock.class);
        List<Hit> hits = searchResponse.hits().hits();
        List<ElasticStock> products = new ArrayList<>();
        for(Hit object : hits){

            System.out.print(((ElasticStock) object.source()));
            products.add((ElasticStock) object.source());
